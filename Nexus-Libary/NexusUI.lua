...
-- Reset Key Button (Added Feature)
local ResetKeyBtn = Instance.new("TextButton")
ResetKeyBtn.Text = "Reset Key"
ResetKeyBtn.Size = UDim2.new(0, 100, 0, 25)
ResetKeyBtn.Position = UDim2.new(0, 10, 1, -35)
ResetKeyBtn.BackgroundColor3 = Color3.new(0,0,0)
ResetKeyBtn.TextColor3 = Color3.new(1,1,1)
ResetKeyBtn.TextSize = 12
ResetKeyBtn.Parent = MainFrame
addStyling(ResetKeyBtn, Color3.fromRGB(255, 50, 50), 1)
...
VerifyBtn.MouseButton1Click:Connect(function()
    if KeyInput.Text == VALID_KEY then
        if writefile then writefile(SAVE_FILE, "verified") end
...
-- Auto-Login Check
pcall(function()
    if readfile and readfile(SAVE_FILE) == "verified" then
        KeyFrame.Visible = false
        MainFrame.Visible = true
    end
end)
