...
local function createMainGUI()
    ...
    for i, tabName in ipairs(tabs) do
        ...
        -- Add Reset Key button to Settings tab
        if tabName == "Settings" then
            local resetKeyBtn = Instance.new("TextButton")
            resetKeyBtn.Size = UDim2.new(1, -20, 0, 50)
            resetKeyBtn.Position = UDim2.new(0, 10, 0, 120)
            resetKeyBtn.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
            resetKeyBtn.Text = "🔑 Reset Key"
            resetKeyBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
            resetKeyBtn.TextSize = 18
            resetKeyBtn.Font = Enum.Font.GothamBold
            resetKeyBtn.Parent = contentPage
            createStroke(resetKeyBtn, CONFIG.Colors.Red, 2)
            createCorner(resetKeyBtn, 8)

            local resetKeyDesc = Instance.new("TextLabel")
            resetKeyDesc.Size = UDim2.new(1, -20, 0, 40)
            resetKeyDesc.Position = UDim2.new(0, 10, 0, 180)
            resetKeyDesc.BackgroundTransparency = 1
            resetKeyDesc.Text = "Clear saved key and return to key verification screen"
            resetKeyDesc.TextColor3 = Color3.fromRGB(150, 150, 150)
            resetKeyDesc.TextSize = 12
            resetKeyDesc.Font = Enum.Font.Gotham
            resetKeyDesc.TextWrapped = true
            resetKeyDesc.Parent = contentPage

            -- Hover effect for reset button
            resetKeyBtn.MouseEnter:Connect(function()
                tweenObject(resetKeyBtn, {BackgroundColor3 = Color3.fromRGB(45, 35, 35)}, 0.2)
            end)
            resetKeyBtn.MouseLeave:Connect(function()
                tweenObject(resetKeyBtn, {BackgroundColor3 = Color3.fromRGB(35, 35, 35)}, 0.2)
            end)

            -- Reset key functionality
            resetKeyBtn.MouseButton1Click:Connect(function()
                -- Create confirmation dialog
                local resetConfirmFrame = Instance.new("Frame")
                resetConfirmFrame.Name = "ResetConfirmFrame"
                resetConfirmFrame.Size = UDim2.new(0, 350, 0, 200)
                resetConfirmFrame.Position = UDim2.new(0.5, -175, 0.5, -100)
                resetConfirmFrame.BackgroundColor3 = CONFIG.Colors.Background
                resetConfirmFrame.BackgroundTransparency = 0.08
                resetConfirmFrame.Parent = screenGui
                createStroke(resetConfirmFrame, CONFIG.Colors.Red, 3)
                createCorner(resetConfirmFrame, 12)

                local resetHeader = Instance.new("TextLabel")
                resetHeader.Size = UDim2.new(1, -40, 0, 40)
                resetHeader.Position = UDim2.new(0, 20, 0, 15)
                resetHeader.BackgroundTransparency = 1
                resetHeader.Text = "⚠️ Reset Key?"
                resetHeader.TextColor3 = Color3.fromRGB(255, 255, 255)
                resetHeader.TextSize = 20
                resetHeader.Font = Enum.Font.GothamBold
                resetHeader.Parent = resetConfirmFrame

                local resetDesc = Instance.new("TextLabel")
                resetDesc.Size = UDim2.new(1, -40, 0, 70)
                resetDesc.Position = UDim2.new(0, 20, 0, 55)
                resetDesc.BackgroundTransparency = 1
                resetDesc.Text = "This will delete your saved key and you'll need to verify again.\n\nAre you sure?"
                resetDesc.TextColor3 = Color3.fromRGB(200, 200, 200)
                resetDesc.TextSize = 14
                resetDesc.Font = Enum.Font.Gotham
                resetDesc.TextWrapped = true
                resetDesc.Parent = resetConfirmFrame

                local confirmResetBtn = Instance.new("TextButton")
                confirmResetBtn.Size = UDim2.new(0, 140, 0, 40)
                confirmResetBtn.Position = UDim2.new(0, 20, 1, -55)
                confirmResetBtn.BackgroundColor3 = CONFIG.Colors.Background
                confirmResetBtn.Text = "Yes, Reset"
                confirmResetBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
                confirmResetBtn.TextSize = 16
                confirmResetBtn.Font = Enum.Font.GothamBold
                confirmResetBtn.Parent = resetConfirmFrame
                createStroke(confirmResetBtn, CONFIG.Colors.Red, 2)
                createCorner(confirmResetBtn, 8)

                local cancelResetBtn = Instance.new("TextButton")
                cancelResetBtn.Size = UDim2.new(0, 140, 0, 40)
                cancelResetBtn.Position = UDim2.new(1, -160, 1, -55)
                cancelResetBtn.BackgroundColor3 = CONFIG.Colors.Background
                cancelResetBtn.Text = "Cancel"
                cancelResetBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
                cancelResetBtn.TextSize = 16
                cancelResetBtn.Font = Enum.Font.GothamBold
                cancelResetBtn.Parent = resetConfirmFrame
                createStroke(cancelResetBtn, CONFIG.Colors.Green, 2)
                createCorner(cancelResetBtn, 8)

                -- Animate in
                resetConfirmFrame.Size = UDim2.new(0, 0, 0, 0)
                tweenObject(resetConfirmFrame, {Size = UDim2.new(0, 350, 0, 200)}, 0.3, Enum.EasingStyle.Back)

                confirmResetBtn.MouseButton1Click:Connect(function()
                    -- Delete the saved key file
                    pcall(function()
                        delfile(CONFIG.KeySaveFile .. ".txt")
                    end)

                    tweenObject(resetConfirmFrame, {Size = UDim2.new(0, 0, 0, 0)}, 0.2).Completed:Connect(function()
                        resetConfirmFrame:Destroy()
                        
                        -- Close main GUI and reload
                        tweenObject(mainFrame, {
                            ...
                            -- Reload the script to show key system again
                            wait(0.5)
                            loadstring(game:HttpGet("YOUR_SCRIPT_URL_HERE"))() -- Replace with your script URL or just reload
                            
                            -- Alternative: Show a message to the player
                            game.StarterGui:SetCore("SendNotification", {
                                Title = "Key Reset";
                                Text = "Key has been reset! Please re-execute the script.";
                                Duration = 5;
                            })
                        end)
                    end)
                ...
                cancelResetBtn.MouseButton1Click:Connect(function()
                    tweenObject(resetConfirmFrame, {Size = UDim2.new(0, 0, 0, 0)}, 0.2).Completed:Connect(function()
                        resetConfirmFrame:Destroy()
                    end)
                end)
